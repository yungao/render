...
type Render interface {
	...
	ProgressDownload(callback func(start, send int64), file string, filename ...string)
}
...
func (r *renderer) Download(file string, filename ...string) {
	agent := strings.ToUpper(r.req.Header.Get("User-Agent"))
	if !strings.Contains(agent, "IPHONE") && !strings.Contains(agent, "IPAD") {
		// iOS原生播放器不能设置此字段
	...
	if strings.Contains(agent, "MSIE") || strings.Contains(agent, "TRIDENT") || strings.Contains(agent, "EDGE") {
		name = url.QueryEscape(name)
		name = strings.Replace(name, "+", "%20", -1)
	}
	r.Header().Set("Content-Disposition", "attachment; filename=\""+name+"\"")

	// fmt.Printf("Agent[%s] download file name: %s \n", agent, name)

	// r.Header().Set("Content-Description", "File Transfer")
	// fmt.Println("Download header[Content-Type]: ", r.Header().Get(ContentType))
	// r.Header().Set("Content-Type", "application/octet-stream")
...
func (r *renderer) ProgressDownload(callbakFunc func(start, send int64), file string, filename ...string) {
	agent := strings.ToUpper(r.req.Header.Get("User-Agent"))
	if !strings.Contains(agent, "IPHONE") && !strings.Contains(agent, "IPAD") {
		// iOS原生播放器不能设置此字段
		r.Header().Set("Content-Type", "application/octet-stream")
